...
## 版本更新
- `v1.0.1` (2026-04-10)
- 新增可配置登录重试策略：支持自定义“重试次数”和“重试间隔(秒)”。
- 开机首次登录失败后自动重试 1 次，仍失败时弹窗提示，同时程序继续后台重连。
- 托盘“立即测试登录”改为使用重试机制并给出成功/失败提示。
- 路径逻辑统一为程序目录绝对路径（配置、日志）并兼容 PyInstaller 资源路径。
- 支持开机启动延时配置（0-3600 秒），减少开机早期网络未就绪导致的失败。

## 功能特点
- 自动检测网络状态，离线时自动触发登录。
- 适配 ePortal 的常见跳转场景（包括 JS 强转页面）。
- 系统托盘常驻，支持手动测试登录与设置面板。
- 支持重试次数/间隔自定义，适配不同网络波动环境。
- 支持开机延时自定义（0-3600 秒），适配不同设备网络初始化速度。
- 防多开机制，避免重复启动冲突。
